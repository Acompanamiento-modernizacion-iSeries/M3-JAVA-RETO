...
public class CuentaBasica extends Cuenta {
    private static final BigDecimal CostoCajeroAutomatico = new BigDecimal("2");
    ...
    public CuentaBasica(Integer numeroCuenta, BigDecimal saldo) {
    ...
    public void retiroCajero(BigDecimal cantidad) {
        if (cantidad.compareTo(BigDecimal.ZERO) > 0 && cantidad.add(CostoRetiroCajero).compareTo(saldo) <= 0) {
            ...
            System.out.println("Costo Retiro en cajero automático: " + CostoRetiroCajero);
    ...
    public void depositoCajeroAutomatico(BigDecimal cantidad) {
        if (cantidad.compareTo(BigDecimal.ZERO) > 0) {
            System.out.println("Saldo Antes del Deposito: " + saldo);
            System.out.println("Depósito en cajero automático realizado. Costo: " + CostoCajeroAutomatico );
            saldo = saldo.add(cantidad).subtract(CostoCajeroAutomatico);
