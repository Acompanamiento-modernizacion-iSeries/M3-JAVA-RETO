...
public class CuentaBasica extends Cuenta{
    ...
    public CuentaBasica(BigDecimal saldo, String numeroCuenta, String titular) {
    ...
    public  boolean retirar(BigDecimal monto, double costo){
        BigDecimal montoTemporal = monto;
        if (monto.compareTo(BigDecimal.ZERO) <= 0){
            Util.mensaje(" *** ALERTA *** \n    - Transacción No exitosa, El monto debe ser mayor a Cero (0).");
            return false;
        }
        montoTemporal.add(BigDecimal.valueOf(costo));
        if (monto.compareTo(montoTemporal) > 0){
            Util.mensaje(" *** ALERTA *** \n    - Transacción No exitosa, Saldo Insuficiente.");
            return false;
        }

        saldo = saldo.subtract(montoTemporal);
